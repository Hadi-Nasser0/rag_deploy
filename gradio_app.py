...
def get_local_ip() -> str:
    ...
    This attempts to connect to an external host (Google DNS) to discover the
    outbound interface IP. If that fails, it falls back to ``127.0.0.1``.
    """
    try:
        # Use a UDP socket to avoid actual network traffic.
...
if __name__ == "__main__":
    # Bind to all interfaces but display the reachable LAN IP for convenience.
    local_ip = get_local_ip()
    demo.launch(server_name="0.0.0.0", server_port=7860, share=False)
    print(f"\nAccess the app locally at http://{local_ip}:7860\n")
